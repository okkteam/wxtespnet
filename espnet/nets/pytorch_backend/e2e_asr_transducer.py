...
class Reporter(chainer.Chain):
    ...
    def report(self, loss, loss_rnnt, loss_att, cer, wer):
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.dtype == "transformer":
            ...
            pred_pad, pred_att, _ = self.decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)
        else:
        ...
        if not math.isnan(loss_data):
            self.reporter.report(loss_data, loss_rnnt_data, loss_att_data, cer, wer)
        else:
        ...
        if (
            ...
            with torch.no_grad():
                ...
                for name, m in self.named_modules():
                    if isinstance(m, MultiHeadedAttention) or isinstance(m, AttAdd):                          
