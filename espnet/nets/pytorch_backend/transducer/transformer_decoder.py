...
class Decoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.lin_att_out = torch.nn.Linear(idim, odim)
    ...
    def forward(self, tgt, tgt_mask, memory, hlens):
        ...
        embed_tgt, tgt_mask = self.decoders(embed_tgt, tgt_mask)
        embed_tgt = self.after_norm(embed_tgt)
        
        h_enc = memory.unsqueeze(2)
        h_dec = embed_tgt.unsqueeze(1)
        ...
        hlens_inverse = hlens != 1
        hlens_inverse.int()
        c_all = []        
        for i in six.moves.range(olength):
            att_c, att_w = self.att[0](
                memory, hlens_inverse, embed_tgt[:, i, :], att_w)

            c_all.append(att_c.unsqueeze(1))

        # (batch x olength) x idim
        c_all = torch.stack(c_all, dim=1).view(batch * olength, -1).cuda()

        # (batch x olength) x odim
        y = self.lin_att_out(c_all)
