...
from espnet.nets.pytorch_backend.transformer.mask import encoder_mask
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, xs, masks):
        ...
        """
        # vgg2l: masks:[8, 1, 393]->[8, 1, 65]
        # masks after encoders:[8, 1, 65]
        if isinstance(self.embed, (Conv2dSubsampling, VGG2L)):
            xs, masks = self.embed(xs, masks)
            #masks_att = encoder_mask(masks)            
        else:
            xs = self.embed(xs)
        #xs, masks_att = self.encoders(xs, masks_att)
